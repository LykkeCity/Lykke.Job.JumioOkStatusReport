...
using Lykke.Job.JumioOkStatusReport.Settings;
using Lykke.Job.JumioOkStatusReport.Modules;
using Lykke.Logs;
...
namespace Lykke.Job.JumioOkStatusReport
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            try
            {
                ...
                {
                    options.DefaultLykkeConfiguration("v1", "JumioOkStatusReport API");
                });
                ...
                builder.RegisterModule(new JobModule(appSettings.CurrentValue.JumioOkStatusReportJob, appSettings.Nested(x => x.JumioOkStatusReportJob), Log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            try
            {
                ...
                app.UseLykkeMiddleware("JumioOkStatusReport", ex => new ErrorResponse {ErrorMessage = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, IReloadingManager<AppSettings> settings)
        {
            ...
            var dbLogConnectionStringManager = settings.Nested(x => x.JumioOkStatusReportJob.Db.LogsConnString);
            ...
            var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                AzureTableStorage<LogEntity>.Create(dbLogConnectionStringManager, "JumioOkStatusReportLog", consoleLogger),
                consoleLogger);
